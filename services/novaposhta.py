...
class NovaPoshtaAPI:
	def __init__(self, api_key):
		...
		# One session per API instance - reuses TCP connection
		# Safe because each sync creates a new NovaPoshtaAPI instance
	...
	def create_internet_document(self, sender_data, recipient_data, package_data):
		...
		logger.debug(f"Full API payload: {json.dumps(payload, indent=2, ensure_ascii=False)}")
